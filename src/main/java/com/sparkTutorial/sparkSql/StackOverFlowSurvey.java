...
public class StackOverFlowSurvey {
	...
	public static void main(String[] args) throws Exception {
		...
		Dataset<Row> responseWithSalaryBucket = castedResponse.withColumn(SALARY_MIDPOINT_BUCKET,
				col(SALARY_MIDPOINT).divide(20000).cast("integer").multiply(20000));

		System.out.println("=== With salary bucket column ===");
		responseWithSalaryBucket.show();
		responseWithSalaryBucket.select(col(SALARY_MIDPOINT), col(SALARY_MIDPOINT_BUCKET))
				.filter(col(SALARY_MIDPOINT).isNotNull()).filter(col(SALARY_MIDPOINT_BUCKET).$greater(0)).show();
